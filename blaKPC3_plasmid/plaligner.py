...
workdir: '/proj/julianog/users/ChristianP/creWGS/blaKPC3_plasmid/'
REF = '/proj/julianog/refs/NZ_CP008826/NZ_CP008826.fa'
readWD = '/proj/julianog/users/ChristianP/creWGS/blaKPC3_plasmid/'
SAMPLES, = glob_wildcards('/proj/julianog/users/ChristianP/creWGS/blaKPC3_plasmid/symlinks/{sample}_R1.fq.gz')
...
rule all:
	...
	#input: 'variants/bla.pass.vcf'
	input: expand('coverage/genomecov/{sample}.cov', sample = SAMPLES)
	#input: expand('variants/{list}.pass.vcf', list = 'st131 nonst131'.split())



#rule remove_non_entries :
#	input: 'variants/{list}.pass.vcf.tmp'
#	output: 'variants/{list}.pass.vcf'
#	shell: 'grep -vP "PL\t0" {input} | grep -vP "\tGT\t." > {output}'

rule split_vcf :
	input: vcf = 'variants/bla.pass.vcf', names = 'names/{list}.list'
	output: 'variants/{list}.pass.vcf.tmp'
	shell: 'vcf-subset -c {input.names} {input.vcf} > {output}'

rule select_variants :
	input: 'variants/bla.qual.vcf'
	output: 'variants/bla.pass.vcf'
	shell: 'java -jar {GATK} \
		-T SelectVariants \
...
rule filter_variants :
	input: vcf = 'variants/bla.vcf', depth = 'intervals/bla.5x@100%.intervals'
	output: 'variants/bla.qual.vcf'
	shell: 'java -jar {GATK} \
		-T VariantFiltration \
		-R {REF} \
		-V {input.vcf} \
...
rule filter_min_depth :
	input: 'variants/bla.vcf'
	output: 'intervals/bla.5x@100%.intervals'
...
rule unified_genotyper :
	input: 'names/bla.list'
	output: 'variants/bla.vcf'
...
rule per_base_coverage:
	...
	output: 'coverage/genomecov/{sample}.cov'
	shell: 'bedtools genomecov -ibam {input} -d > {output}'
		# -d reports per-base coverage

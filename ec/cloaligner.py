...
workdir: '/proj/julianog/users/ChristianP/creWGS/ec/'
REF = '/proj/julianog/refs/EcNCTC9394/EcNCTC9394.fa'
readWD = '/proj/julianog/users/ChristianP/creWGS/ec/'
SAMPLES, = glob_wildcards('/proj/julianog/users/ChristianP/creWGS/ec/symlinks/{sample}_R1.fq.gz')
...
rule split_vcf :
	input: vcf = 'variants/ec.pass.vcf', names = 'names/{list}.list'
	output: 'variants/{list}.pass.vcf.tmp'
	shell: 'vcf-subset -c {input.names} {input.vcf} > {output}'

rule select_variants :
	input: 'variants/ec.qual.vcf'
	output: 'variants/ec.pass.vcf'
	shell: 'java -jar {GATK} \
		-T SelectVariants \
...
rule filter_variants :
	input: vcf = 'variants/ec.vcf', depth = 'intervals/ec.5x@100%.intervals'
	output: 'variants/ec.qual.vcf'
	shell: 'java -jar {GATK} \
		-T VariantFiltration \
		-R {REF} \
		-V {input.vcf} \
...
rule filter_min_depth :
	input: 'variants/ec.vcf'
	output: 'intervals/ec.5x@100%.intervals'
